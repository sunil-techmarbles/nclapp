...
<div class="abs" style="text-align: center;">
	<table class="table record-filed" cellspacing="0" cellpadding="0" border="0">
		<tbody>
			<tr>
				<td>
					<a class="btn btn-primary btn-sm border" href="{{route('add.supplies',['pageaction' => request()->get('pageaction')])}}" >Add Record</a>
					<a class="btn btn-secondary btn-sm border" href="{{route('export.supplies',['pageaction' => request()->get('pageaction')])}}">Export</a>
				</td>
				<td nowrap="" style="text-align: right;">
					<form method="post" action="{{route('import.supplies',['pageaction' => request()->get('pageaction')])}}" enctype="multipart/form-data">
						@csrf
				...
				<td nowrap="" style="text-align: right">
					<form method="get" action="{{route('supplies')}}">
						...
						@if(request()->get('s') || request()->get('f'))
							<a class="btn btn-dark btn-sm border" href="{{route('supplies',['pageaction' => request()->get('pageaction')])}}">Reset</a>
						@endif
					</form>
				</td>
			</tr>
		...
		<tbody>
			...
			<tr>
				<td nowrap="">
					...
					<a href="{{route('edit.supplies', ['pageaction' => request()->get('pageaction'),'id' => $supplie->id])}}">
