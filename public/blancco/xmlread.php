...
$single_pdf_report_request_filePath_updated = __DIR__."/". BLANCCO_XML_REQUEST_FOLDER . "/updated_xml_request.xml"; 
$single_pdf_report_request_filename_updated = BLANCCO_XML_REQUEST_FOLDER .'/updated_xml_request.xml';  
...
foreach ( $reports as $key => $report ) {    
		...
		if( in_array( $state, $report_allowed_states ) ) {
			if( !empty( $serial ) && !empty( $asset_id ) && !empty( $lot_number ) ) {
				...
				TMdoCurlRequest( $single_pdf_report_request_filePath_updated , $single_pdf_report_request_filename_updated , 'singlexml' , "xml" , $file_name, $reportUuid );     
